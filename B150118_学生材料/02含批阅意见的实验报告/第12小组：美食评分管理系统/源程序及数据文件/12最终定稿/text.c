...
void countManage(Food foo[],int n)               /*该函数完成项目评分统计功能*/
{
		int choice;
		double mark[3][3];
		do
...
int runMain(Food foo[],int n,int choice)    /*主控模块，对应于一级菜单其下各功能选择执行*/
{
		switch(choice)
		{
			...
			case 3: scoreManage(foo,n);     /* 3. 菜品评分管理*/
					break;
			case 4: countManage(foo,n);     /* 4. 项目评分统计*/
					break;
