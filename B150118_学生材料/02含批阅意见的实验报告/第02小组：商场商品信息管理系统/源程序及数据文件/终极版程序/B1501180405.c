...
{                 /*形式参数k代表输出不同的内容，0、1分别对应库存较多、较少*/

          printf(s);                                  /*这里的s传入的是输出库存的提示信息*/
		  printf("%10d",m[k]);
	      printf("\n");
}
...
void countManage(Product pro[],int n)               /*该函数完成商品库存统计功能*/
{
		...
		{
			menuCount( );                        /*显示对应的二级菜单*/
			calcuMark(mark,pro,n);                 /*调用此函数求库存较多及较少商品*/
			printf("choose one operation you want to do:\n");
			scanf("%d",&choice);
			switch(choice)
			{
				case 1:  printMarkCourse("较多库存是:\n",mark,0);
			              break;
				case 2:  printMarkCourse("较少库存是:\n",mark,1);
				          break;
				case 0:   break;
			}
		}while (choice);
...
/*按编号进行插入删除修改，编号不能重复*/
{
  ...
  printf("choose one operation you want to do:\n");
		     ...
		     switch(choice)
		     {
			   case 1:	 readPro(&s,1);       /*读入一条待插入的商品记录,初始化s*/
...
int readPro(Product  *pro , int n)          /*读入商品记录值，编号为0或读满规定条数记录时停止*/
{
	...
	for (i=0;i<n;i++)
	{
		...
		printf("brand:  ");
		scanf("%s",pro[i].brand);
    	    pro[i].total=0.0;                /*总价需要计算求得，初值置为0.0*/
...
int equal(Product s1,Product s2,int condition)  /*如何判断两个Product记录相等*/
{
 ...
 else if (condition==3)                /*如果参数condition的值为3，则比较品牌*/
	 {
	     if (strcmp(s1.brand,s2.brand)==0) 	return 1;
		else return 0;
     }
 else if (condition==4)                /*如果参数condition的值为4，则比较价格*/
...
void calcuRank(Product pro[],int n)          /*根据价格计算所有商品的排名，价格相同者名次相同*/
{
	...
	{
		if (equal(pro[i],pro[i-1],4))         /*当前记录与其相邻的前一条记录如果价格相等*/
